...
{  
	...
	{  
		...
		{  
			...
			{  
				memset(&tmp,0,sizeof(tmp));  
				sprintf(tmp,"<html><head><title>404 Not Found</title></head><body bgcolor=\"white\"><center><h1>404 Not Found</h1></center><hr><center>Powered by %s</center></body></html>","fasthttp");  
			...
			int len = filesize(filepath);
			do{
				memset(&tmp2,0,sizeof(tmp2));  
				int readlen = read(fd,tmp2,len);  
				if(readlen == -1){
					if(errno == EINTR)
						continue;
					else
				...
				write(client_sock,tmp2,readlen);  
				len -= readlen;
			}while(len);
			close(fd);  
			close(client_sock);  
...
{  
	...
	printf("%s\n", __func__);
#endif
	for (i = 0;s[i] != '\n';i++){        /* obtain the first line in http protocol head */  
#ifdef DEBUG
		printf("%c", s[i]);  
#endif
	}
